...
Необходимо уменьшить потребление памяти postgres-сервером:

Фактический максимальный объем ОЗУ можно получить по формуле = shared_buffers+(temp_buffers+ work_mem)*max_connections

- В качестве приблизительного ориентира shared_buffers следует установить в пределах от 25% до 40% памяти, которую вы готовы использовать для PostgreSQL, 
- max_connections - максимальное количество параллельных подключений, которые вы хотите иметь. 
- Настроить параметр work_mem, который задаёт объём памяти, который будет использоваться для внутренних операций сортировки и хеш-таблиц, прежде чем будут задействованы временные файлы на диске
- Выставить максимальное временных буферов которые будет использовать БД в temp_buffers. 
- Параметры temp_buffers и work_mem необходимо настроить так, чтобы они не превышали свой бюджет ОЗУ, установка temp_buffers довольно низкого значения (по умолчанию 8 МБ) позволит становить work_mem немного больше. 
- Так же следует включить использование огромных страниц памяти, через huge_pages с параметрами on или try - желательно его, потому как сервер попытается использовать огромные страницы, но может переключиться на обычные, если это не удастся. Со значением on, если использовать огромные страницы не получится, сервер не будет запущен. Со значением off огромные страницы использоваться не будут.


Со стороны ОС можно настроить рамки предвыделения оперативной памяти и ограничить размер выделяемой памяти
```
